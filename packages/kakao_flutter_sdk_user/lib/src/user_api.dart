...
class UserApi {
  ...
  /// [prompts]에 동의 화면에 상호작용 추가 요청 프롬프트 전달<br>
  /// [channelPublicIds]에 카카오톡 채널 프로필 ID 전달<br>
  /// [serviceTerms] 서비스 약관 목록 전달<br>
  /// ID 토큰 재생 공격 방지를 위한 검증 값, 임의의 문자열은 [nonce]에 전달<br>
  /// [settleId]에 정산 ID 전달<br>
  /// <br>
  /// EN: Certification Login with Kakao Talk<br>
  /// Pass data to sign to [signData]<br>
  /// Pass the prompts to [prompts] for requests to add an interaction to the consent screen<br>
  /// Pass Kakao Talk Channel's profile IDs to [channelPublicIds]<br>
  /// Pass List of service terms to [serviceTerms]<br>
  /// Pass a random string to prevent replay attacks to [nonce]<br>
  /// Pass the settlement ID to [settleId]
  ...
  /// KO: 카카오계정으로 인증 로그인<br>
  /// [signData]에 서명할 데이터 전달<br>
  /// [prompts]에 상호작용 추가 요청 프롬프트 전달<br>
  /// [channelPublicIds]에 카카오톡 채널 프로필 ID 전달<br>
  /// [serviceTerms] 서비스 약관 목록 전달<br>
  /// [loginHint]에 카카오계정 로그인 페이지의 ID란에 자동 입력할 이메일 또는 전화번호 전달, +82 00-0000-0000 형식<br>
  /// ID 토큰 재생 공격 방지를 위한 검증 값, 임의의 문자열은 [nonce]에 전달<br>
  /// [settleId]에 정산 ID 전달<br>
  /// <br>
  /// EN: Certification Login with Kakao Account<br>
  /// Pass data to sign to [signData]<br>
  /// Pass the prompts to [prompts] for requests to add interactions<br>
  /// Pass Kakao Talk Channel's profile IDs to [channelPublicIds]<br>
  /// Pass List of service terms to [serviceTerms]<br>
  /// Pass [loginHint] an email or phone number in the format +82 00-0000-0000 to fill in the ID field of the Kakao Account login page<br>
  /// Pass a random string to prevent replay attacks to [nonce]<br>
  /// Pass the settlement ID to [settleId]
